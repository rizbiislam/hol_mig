...
def sop_default_rules():
    ...
    batch("Zonelnfold", note="SOP: one fixed Id per pourashava from the 'ZoneInfoId' reference file. CONFIRMED for Durgapur = 145 (cross-checked from both Durgapur-Ward.xlsx and Durgapur-Street.xlsx, ZoneInfoId column is 145 on every row of both). Pre-filled, but verify it's still current before shipping.")
    ...
    manual("BankAccountlnfold", note="SOP: create Bank records in the live system from source info, then query the DB for the generated Id. Some pourashavas already have bank info pre-created — check before creating duplicates. Map a source column here only once you have the resolved Ids.")
